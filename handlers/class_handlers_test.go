...
func TestCreateClassHandler(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "/classes", reqBody)
	if err != nil {
		t.Fatalf("error creating request: %s", err)
	}
	...
	if err != nil {
		...
		th.AssertEqual(t, "teaching grade", classApi.TeachingGrade, 9)
		th.AssertEqual(t, "profID", classApi.ProfessorID, "123456789012345678901234567890")
		th.AssertEqual(t, "subject", classApi.Subject, "math")
		if th.AssertEqual(t, "roster len", len(classApi.Roster), 2) {
			th.AssertEqual(t, "roster[0]", classApi.Roster[0], "123456789012345678901234567890")
	...
	defer students.DeleteClass(classApi.ClassID)
}
...
func TestGetClassHandler(t *testing.T) {
	...
	// create fatal cause needs ID for rest
	createdClass, err := students.CreateClass(class.TeachingGrade, class.ProfessorID, class.Subject, class.Roster)
	if err != nil {
		t.Fatal(err)
	}
	requestString := fmt.Sprintf("/class/%s", createdClass.ClassID)
	...
	// create a router and register the handler
	r := mux.NewRouter()
	r.HandleFunc("/class/{class_id}", GetClassHandler).Methods("GET")
	...
	//assert.Equal(t, "123456789012345678901234567890", respBody["class_id"])
	defer students.DeleteClass(class.ClassID)
}
...
func Test_UpdateClass(t *testing.T) {
	...
	// build request
	requestString := fmt.Sprintf("/classes/%s", createdClass.ClassID)
	reqUPDATE, err := http.NewRequest("PUT", requestString, reqBody)
	if err != nil {
		t.Fatal(err)
	}
	// reminder to stop forgetting to set your header
	reqUPDATE.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	reqUPDATE.Header.Set("Content-Length", strconv.Itoa(len(form.Encode())))
	r.ServeHTTP(rr, reqUPDATE)
	// Check the response status code
	...
	classUpdate, err := students.GetClass(createdClass.ClassID)
	if err != nil {
		t.Error(err)
	} else {
		th.AssertEqual(t, "professorid", classUpdate.ProfessorID, "123456789012345678901234567895")
	...
	defer students.DeleteClass(createdClass.ClassID)
}
